...
//Function for getting the size of data from matrix
int getdatalength(char* file_name);
...
int main(int argc, char** argv) {
	...
	data_sizeM = getdatalength(argv[1]);
	data_sizeN = getdatalength(argv[2]);
	if(data_sizeM != 0)
        	M  = AllocateMatrix(sqrt(data_sizeM), sqrt(data_sizeM), 0);
	else
		M  = AllocateMatrix(MATRIX_SIZE, MATRIX_SIZE, 0);
	std::cout<<"Size of M "<<data_sizeM<<std::endl;
	cout<<"Height of M"<<M.width<<endl;
	N = AllocateMatrix(M.height, M.width, 0);//Since we are dealing with square matrices
        P  = AllocateMatrix(M.height, M.width, 0);
        errorM = ReadFile(&M, argv[1]);
	std::cout<<"Data Read "<<errorM<<std::endl;
    ...
    Matrix reference = AllocateMatrix(M.height, M.width, 0);
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    //Copying matrix from host to device
    CopyToDeviceMatrix(d_M, M);
    CopyToDeviceMatrix(d_N, N);
    
    //Defining number of blocks and number of threads
    dim3 block, grid;
    block.x = 2;
    block.y = 2;
    block.z = 1;
    grid.x = ceil(d_M.width/2.0);
    cout<<"Grid x "<<grid.x<<endl;
    grid.y = ceil(d_M.height/2.0);
    grid.z = 1;

    //Calling kernel matrix multicplication function
    MatrixMulKernel<<<grid, block>>>(d_M, d_N, d_P);
...
Matrix AllocateDeviceMatrix(const Matrix M)
{
    ...
    cout<<"Matrix width "<<M.width<<endl;
...
int ReadFile(Matrix* M, char* file_name)
{
    ...
    for (i = 0; i < data_read; i++){
        int a = fscanf(input, "%f", &(M->elements[i]));
	//cout<<"fscan "<<a<<endl;
    }
    return data_read;
}
...
int getdatalength(char* file_name)
{
    ...
    float* temp_memory;
    temp_memory = (float*) malloc(1*sizeof(float)); 
    while(1)
    {
	int a = fscanf(input, "%f",&(temp_memory[0]));
	if(a ==1)
	    count++;
	else
	    break;
