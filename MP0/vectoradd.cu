...
void VectorAddOnDevice(const Vector A, const Vector B, Vector C)
{
	...
	std::cout<<"Length of vector "<<d_A.length<<std::endl;
	dim3 DimGrid(d_A.length/256, 1, 1);
 	if (d_A.length%256) DimGrid.x++;
  		dim3 DimBlock(256, 1, 1);	
	VectorAddKernel<<<ceil(d_A.length/256.0),256>>>(d_A, d_B, d_C);
	std::cout<<"Check 1"<<std::endl;
	CopyFromDeviceVector(C, d_C);
	//std::cout<<"Ouput "<<d_C.elements<<std::endl;
